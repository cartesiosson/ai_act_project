...
# Load JSON-LD context from file (generated by generate_context.sh)
_jsonld_context = None

def get_jsonld_context():
    """Load JSON-LD context from file to ensure proper URI resolution in Docker"""
    global _jsonld_context
    if _jsonld_context is None:
        context_path = os.environ.get("JSONLD_CONTEXT_PATH", "/ontologias/json-ld-context.json")
        try:
            with open(context_path, 'r') as f:
                ctx_file = json.load(f)
                _jsonld_context = ctx_file.get("@context", ctx_file)
            print(f"JSON-LD context loaded from {context_path}")
        except Exception as e:
            print(f"Warning: Could not load JSON-LD context from {context_path}: {e}")
            # Fallback to minimal inline context
            _jsonld_context = {
...
async def create_system(json_ld: IntelligentSystem, db=Depends(get_database)):
    ...
    try:
        ...
        # NO cerrar la conexión - esto rompería todas las peticiones futuras
    ...
    # y expandir a URIs completas para que JSON-LD los procese correctamente
    AI_BASE = "http://ai-act.eu/ai#"

    def expand_to_uri(v):
        """Expand value to full URI - handles both 'ai:X' and 'X' formats"""
        if not isinstance(v, str):
            return v
        if v.startswith("http://") or v.startswith("https://"):
            return {"@id": v}  # Already a full URI
        if v.startswith("ai:"):
            return {"@id": AI_BASE + v[3:]}  # Remove 'ai:' prefix and expand
        # Value without prefix - assume it's an ai: entity
        return {"@id": AI_BASE + v}
    ...
    for multivalue_key in [
        ...
        "ai:hasSystemCapabilityCriteria", "ai:hasCriteria", "ai:hasComplianceRequirement",
        "ai:hasGPAIClassification", "ai:hasProhibitedPractice", "ai:hasLegalException"
    ]:
        if multivalue_key in mapped:
            ...
            # Expand all values to full URIs so JSON-LD treats them as @id references
            mapped[multivalue_key] = [expand_to_uri(v) for v in mapped[multivalue_key] if v]

    # Also expand single-value properties
    if "ai:hasRiskLevel" in mapped and isinstance(mapped["ai:hasRiskLevel"], str):
        mapped["ai:hasRiskLevel"] = expand_to_uri(mapped["ai:hasRiskLevel"])

    # FIX: Use context from file to ensure ai: prefix resolves correctly in Docker
    # The external URL http://localhost:8000/static/json-ld-context.json is not accessible from containers
    mapped["@context"] = get_jsonld_context()
...
async def update_system(urn: str, json_ld: IntelligentSystem = Body(...), db=Depends(get_database)):
        ...
        for k, v in json_ld.items():
                if not k.startswith("ai:") and k not in ["@id", "@type", "@context"]:
                        ...
                        mapped[k] = v
        # Expand values to full URIs for JSON-LD processing
        AI_BASE = "http://ai-act.eu/ai#"
