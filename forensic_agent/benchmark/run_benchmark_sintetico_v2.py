#!/usr/bin/env python3
"""
Forensic Agent Benchmark Runner - Version 2
Analyzes synthetic incidents with AIAAIC-aligned distribution.

Distribution based on AIAAIC Repository analysis (n=2,132):
- Transparency: 18% (normalized from 34.8%)
- Accuracy/Reliability: 17% (normalized from 33.1%)
- Privacy/Surveillance: 12% (normalized from 23.7%)
- Safety: 12% (normalized from 22.6%)
- Fairness/Bias: 10% (normalized from 19.9%)
- Mis/disinformation: 8% (normalized from 16.3%)
- Copyright: 4% (normalized from 7.5%)
...
# Import the v2 generator (AIAAIC-aligned)
from generate_synthetic_incidents_v2 import generate_benchmark_dataset


class ForensicBenchmarkV2:
    """Benchmark runner for forensic agent - Version 2 (AIAAIC-aligned)"""
    ...
    def check_health(self) -> bool:
        ...
        try:
            ...
            print(f"ERROR: Forensic agent not available: {e}")
            return False
    ...
    def analyze_incident(self, incident: Dict) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result["expected_risk_level"] = incident["metadata"].get("expected_risk_level", "Unknown")
                result["expected_incident_type"] = incident["metadata"].get("template_type", "unknown")
                return result
            else:
                return {
    ...
    def run_benchmark(self, incidents: List[Dict], max_incidents: int = None):
        ...
        print(f"\n{'='*70}")
        print(f"FORENSIC AGENT BENCHMARK V2 (Synthetic - AIAAIC-aligned)")
        print(f"{'='*70}")
        ...
        for idx, incident in enumerate(incidents, 1):
            ...
            narrative_preview = incident['narrative'][:50] + "..." if len(incident['narrative']) > 50 else incident['narrative']
            print(f"[{idx}/{len(incidents)}] {narrative_preview}", end=" ", flush=True)
            ...
            # Track expected risk levels from ground truth
            expected_risk = incident["metadata"].get("expected_risk_level", "Unknown")
            self.metrics["expected_risk_levels"][expected_risk] = self.metrics["expected_risk_levels"].get(expected_risk, 0) + 1
            ...
            if status == "COMPLETED":
                ...
                # Track risk levels (actual classification)
                ...
                print(f"OK {proc_time:.1f}s (conf: {confidence:.2f}, risk: {risk_level})")
    ...
    def compute_statistics(self) -> Dict:
        ...
        stats = {
            ...
            "distribution_source": "AIAAIC-aligned (normalized)",
            ...
            "expected_risk_distribution": self.metrics["expected_risk_levels"],
    ...
    def save_results(self, output_dir: Path):
        ...
        # Save full results
        results_file = output_dir / f"synthetic_benchmark_results_v2_{timestamp}.json"
        ...
        stats_file = output_dir / f"synthetic_benchmark_stats_v2_{timestamp}.json"
    ...
    def print_report(self, stats: Dict):
        ...
        print(f"\n{'='*70}")
        print(f"BENCHMARK REPORT V2 (AIAAIC-aligned)")
        print(f"{'='*70}\n")
        ...
        # Expected vs Actual Risk Distribution
        print(f"Expected Risk Distribution (Ground Truth):")
        for risk, count in sorted(stats.get("expected_risk_distribution", {}).items()):
            pct = count / summary['total_incidents'] * 100
            print(f"  {risk}: {count} ({pct:.1f}%)")
        print()

        # Actual Risk distribution
        if stats["risk_distribution"]:
            print(f"Actual Risk Level Distribution:")
...
def main():
    import sys
    n_incidents = int(sys.argv[1]) if len(sys.argv) > 1 else 50
    print(f"Generating {n_incidents} synthetic incidents (v2 - AIAAIC-aligned distribution)...")
    incidents = generate_benchmark_dataset(n_incidents)

    # Save generated incidents
    incidents_file = Path(__file__).parent / "benchmark_incidents_v2.json"
    with open(incidents_file, 'w') as f:
        json.dump(incidents, f, indent=2)
    print(f"Generated {len(incidents)} incidents -> {incidents_file}")
    ...
    for inc in incidents:
        t = inc["metadata"]["template_type"]
        r = inc["metadata"]["expected_risk_level"]
        type_counts[t] = type_counts.get(t, 0) + 1
        risk_counts[r] = risk_counts.get(r, 0) + 1

    for t, c in sorted(type_counts.items(), key=lambda x: x[1], reverse=True):
        pct = c / len(incidents) * 100
        print(f"  {t}: {c} ({pct:.1f}%)")
