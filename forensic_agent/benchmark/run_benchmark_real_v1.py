#!/usr/bin/env python3
"""
Forensic Agent Real Benchmark Runner
Analyzes real AI incidents from the AIAAIC Repository

Data source: AIAAIC Repository (https://www.aiaaic.org/aiaaic-repository)
Local CSV: data/AIAAIC Repository - Incidents.csv
"""
...
class AIAAICDataLoader:
    """Loads real incident data from AIAAIC Repository CSV"""
    ...
    def load_incidents(self) -> List[Dict]:
        """
        Load incidents from local AIAAIC CSV file

        CSV Structure (AIAAIC Repository export):
        - Row 1: Title row (skip)
        - Row 2: Headers (AIAAIC ID#, Headline, Occurred, Country(ies), etc.)
        ...
        with open(self.csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                incident = self._parse_row(row_dict)
                if incident:
    ...
    def _parse_row(self, row: Dict) -> Optional[Dict]:
        """
        Parse a CSV row into incident format compatible with forensic agent

        AIAAIC columns:
        - AIAAIC ID#: Unique identifier
        - Headline: Incident title
        - Occurred: Year/date
        - Country(ies): Location
        - Sector(s): Industry sector
        - Deployer(s): Organization using the system
        - Developer(s): Organization that built the system
        - System name(s): Name of AI system
        - Technology(ies): Type of AI technology
        - Purpose(s): What the system does
        - News trigger(s): What caused the news
        - Issue(s): Types of issues (Bias, Safety, Privacy, etc.)
        - Summary/links: URL to full incident page
        """
        # Get headline
        ...
        # Get AIAAIC ID
        aiaaic_id = row.get('AIAAIC ID#', '').strip()
        if not aiaaic_id:
            return None
        ...
        # Add system info
        system_name = row.get('System name(s)', '').strip()
        if system_name:
            narrative_parts.append(f"The AI system involved is {system_name}.")

        # Add technology type
        technology = row.get('Technology(ies)', '').strip()
        if technology:
            narrative_parts.append(f"Technology type: {technology}.")

        # Add purpose
        purpose = row.get('Purpose(s)', '').strip()
        if purpose:
            narrative_parts.append(f"System purpose: {purpose}.")

        # Add deployer/developer
        deployer = row.get('Deployer(s)', '').strip()
        developer = row.get('Developer(s)', '').strip()
        if deployer:
        ...
        summary_links = row.get('Summary/links', '').strip()
...
class RealBenchmark:
    """Benchmark runner for real AIAAIC incidents"""
    ...
    def run_benchmark(self, incidents: List[Dict], num_cases: int = None, randomize: bool = True):
        """
        Run benchmark on selected incidents

        Args:
            incidents: List of all available incidents
            num_cases: Number of cases to analyze (None = all)
            randomize: Whether to randomly select cases
        """
        # Select cases
        ...
        print(f"\n{'='*70}")
        print(f"FORENSIC AGENT REAL BENCHMARK (AIAAIC)")
        print(f"{'='*70}")
        ...
        for idx, incident in enumerate(selected_incidents, 1):
            incident_id = incident['id']
            title = incident['metadata'].get('aiaaic_title', incident_id)[:50]
            ...
            if status == "COMPLETED":
                ...
                confidence = result.get("extraction", {}).get("confidence", {}).get("overall", 0)
                self.metrics["confidence_scores"].append(confidence)
                print(f"Low conf ({confidence:.2f})")
    ...
    def save_results(self, output_dir: Path):
        ...
        # Save full results
        results_file = output_dir / f"real_benchmark_results_v1_{timestamp}.json"
        ...
        stats_file = output_dir / f"real_benchmark_stats_v1_{timestamp}.json"
    ...
    def print_report(self, stats: Dict):
        ...
        print(f"\n{'='*70}")
        print(f"REAL BENCHMARK REPORT V1 (AIAAIC)")
        print(f"{'='*70}\n")
        ...
        print(f"  Version: {AIAAIC_ATTRIBUTION['version']}")
        print()
...
def main():
    ...
    print("="*70)
    print("AIAAIC Real Incident Benchmark V1")
    print("="*70)
    print()

    # Load AIAAIC data from local CSV
    loader = AIAAICDataLoader()
    try:
    ...
    if len(sys.argv) > 1:
        ...
        # Ask user for number of cases interactively
        while True:
            try:
