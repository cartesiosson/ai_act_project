#!/usr/bin/env python3
"""
Forensic Agent Benchmark Runner - Version 1
Analyzes 100 synthetic incidents with improved distribution:
- More privacy_violation and transparency_failure cases
- Less bias (reduced from original)
- Added MinimalRisk cases
- Added appropriation and copyright incident types
"""
...
# Import the v1 generator
from generate_synthetic_incidents_v1 import generate_benchmark_dataset


class ForensicBenchmarkV1:
    """Benchmark runner for forensic agent - Version 1"""
    ...
    def run_benchmark(self, incidents: List[Dict], max_incidents: int = None):
        ...
        print(f"\n{'='*70}")
        print(f"FORENSIC AGENT BENCHMARK V1 (Synthetic)")
        print(f"{'='*70}")
    ...
    def save_results(self, output_dir: Path):
        ...
        # Save full results
        results_file = output_dir / f"benchmark_results_v1_{timestamp}.json"
        ...
        stats_file = output_dir / f"benchmark_stats_v1_{timestamp}.json"
...
def main():
    print("Generating 100 synthetic incidents (v1 - improved distribution)...")
    incidents = generate_benchmark_dataset(100)

    # Save generated incidents
    incidents_file = Path(__file__).parent / "benchmark_incidents_v1.json"
